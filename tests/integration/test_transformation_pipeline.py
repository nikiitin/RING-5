...
class TestTransformationPipeline:
    ...
    def test_single_shaper_transformation(self, sample_benchmark_data: pd.DataFrame) -> None:
        ...
        expected_omnetpp = 1.3 / 1.5  # 0.866...
        assert abs(tx_rows[tx_rows["benchmark"] == "mcf"]["ipc"].iloc[0] - expected_mcf) < 0.01
    
    def test_multi_shaper_pipeline(self, sample_benchmark_data: pd.DataFrame, facade: BackendFacade) -> None:
        """
        ...
        assert all(mcf_idx < omnetpp_idx for mcf_idx in mcf_indices for omnetpp_idx in omnetpp_indices)
    ...
    def test_complex_pipeline_workflow(self, sample_benchmark_data: pd.DataFrame, tmp_path: Path) -> None:
        """
        ...
        # Load into CSV pool
        csv_id = facade.add_to_csv_pool(str(csv_path))
        ...
        pd.testing.assert_frame_equal(
            ...
            retransformed.reset_index(drop=True)
        )

...
class TestDataPipeline:
    ...
    def test_shaper_pipeline_execution(self, sample_data):
        ...
        shaper_configs = [
            {
                ...
                'column': 'group',
                'values': ['A', 'B'] # Filter out C
            },
            {
                'type': 'sort',
                'order_dict': {'value': []} # Implies sorting by value? sort shaper usually sorts by a column based on custom order or values
                # Looking at sort.py implementation might be needed, but assuming standard behavior
            }
        ]
        ...
        filter_shaper = ShaperFactory.createShaper('conditionSelector', {
            'column': 'value',
            'mode': 'less_than',
            'threshold': 50
        })

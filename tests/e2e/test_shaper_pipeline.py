...
# ===================================================================
# Tier 1 -- Pipeline save and load
# ===================================================================


@pytest.mark.xdist_group("e2e_pipeline_save_load")
class TestPipelineSaveLoad:
    """Tier 1: Save and load pipeline configurations.

    Tests are ordered: first create+finalize, then save, then load.
    """

    def test_01_create_and_finalize(self, tier1_page: Page) -> None:
        """Create a plot with a Sort step and finalize the pipeline."""
        mp = ManagePlotsPage(tier1_page)
        _create_bar_plot(mp, "Save Load Test")
        mp.add_shaper("Sort")
        mp.assert_pipeline_step_count(1)
        mp.finalize_pipeline()

    def test_02_save_pipeline(self, tier1_page: Page) -> None:
        """Open the save dialog, enter a name, and save."""
        mp = ManagePlotsPage(tier1_page)
        mp.open_save_dialog()
        expect(mp.save_dialog_name_input).to_be_visible(timeout=E2E_TIMEOUT)
        mp.save_dialog_name_input.fill("test_pipe")
        mp.save_dialog_save_button.click()
        mp.wait_for_streamlit()

    def test_03_load_dialog_shows_pipelines(self, tier1_page: Page) -> None:
        """Open the load dialog and verify at least one pipeline exists."""
        mp = ManagePlotsPage(tier1_page)
        mp.open_load_dialog()
        expect(mp.load_dialog_selector).to_be_visible(timeout=E2E_TIMEOUT)
        # The selector should contain the previously saved pipeline
        expect(mp.load_dialog_selector).to_contain_text("test_pipe", timeout=E2E_TIMEOUT)
        mp.load_dialog_cancel_button.click()
        mp.wait_for_streamlit()

    def test_04_load_pipeline_into_new_plot(self, tier1_page: Page) -> None:
        """Create a new plot and load the saved pipeline into it."""
        mp = ManagePlotsPage(tier1_page)
        _create_bar_plot(mp, "Load Target")
        mp.open_load_dialog()
        expect(mp.load_dialog_selector).to_be_visible(timeout=E2E_TIMEOUT)
        mp.load_dialog_load_button.click()
        mp.wait_for_streamlit()
        # The loaded pipeline should have at least 1 step
        mp.assert_pipeline_step_count(1)

    def test_05_no_pipelines_warning(self, tier1_page: Page) -> None:
        """Verify: if we create a fresh plot, load still finds saved pipes.

        This confirms persistence of saved pipelines across plots.
        """
        mp = ManagePlotsPage(tier1_page)
        _create_bar_plot(mp, "Another Load")
        mp.open_load_dialog()
        # Saved pipelines persist, so selector should still be visible
        expect(mp.load_dialog_selector).to_be_visible(timeout=E2E_TIMEOUT)
        mp.load_dialog_cancel_button.click()
        mp.wait_for_streamlit()

...
@pytest.fixture
def mock_streamlit():
    with patch("src.plotting.types.grouped_bar_plot.st") as mock_st:
        # Mock columns
...
def test_render_config_ui(mock_streamlit, sample_data):
    ...
    mock_streamlit.text_input.side_effect = ["Title", "X", "Y", "Leg"] # title, xlabel, ylabel, legend_title
...
def test_create_figure_with_error_bars_and_filters(sample_data):
    ...
    assert list(layout.xaxis.categoryarray) == ["A"]
...
def test_render_advanced_options_filtering(sample_data):
    ...
    with patch("src.plotting.types.grouped_bar_plot.BasePlot.render_advanced_options") as mock_super:

...
def test_map_parsing_vars_duplicate_error(data_parser):
    ...
    vars_config = [{"id": "var1", "type": "scalar"}, {"id": "var1", "type": "scalar"}]

    with patch("src.utils.utils.checkElementExists"), patch(
        "src.utils.utils.getElementValue", side_effect=lambda obj, key: obj.get(key)
    ):
...
def test_parse_stats_with_files(data_parser):
    ...
    data_parser._parseConfig = [
        {"path": "/path", "files": "*.txt", "vars": [{"id": "v1", "type": "scalar"}]}
    ]
    data_parser._shouldCompress = "False"

    with patch("glob.glob", return_value=["file1.txt"]), patch(
        "src.utils.utils.getElementValue", side_effect=lambda obj, key: obj.get(key)
    ), patch("src.utils.utils.checkElementExists"), patch.object(
        data_parser, "_mapParsingVars", return_value={"v1": MagicMock()}
    ):
...
def test_turn_into_csv_content(data_parser):
    ...
    # Mock result dict
    result = {"scalar_var": mock_scalar, "vector_var": mock_vector}
    ...
    data_parser._results = [{"scalar_var": Scalar(), "vector_var": Vector()}]

"""Tests for SeriesStyleConfig — construction, serialization, immutability."""
...
class TestSeriesStyleSpecCustom:
    ...
    def test_hatching_patterns(self) -> None:
        ...
        for pattern in ["/", "\\", "|", "-", "+", "x", "o", "O", ".", "*"]:
            spec = SeriesStyleConfig(hatching_pattern=pattern)
...
class TestSeriesStyleSpecFrozen:
    ...
    def test_frozen_hatching(self) -> None:
        ...
        spec = SeriesStyleConfig(hatching_pattern="/")
        with pytest.raises(AttributeError):
...
class TestSeriesStyleSpecSerialization:
    ...
    def test_default_round_trip(self) -> None:
        ...
        original = SeriesStyleConfig()
        restored = SeriesStyleConfig.from_dict(original.to_dict())
    ...
    def test_to_dict_produces_plain_dict(self) -> None:
        ...
        spec = SeriesStyleConfig(line_width=3.0, opacity=0.8)
        d = spec.to_dict()
    ...
    def test_from_dict_ignores_unknown_keys(self) -> None:
        ...
        spec = SeriesStyleConfig.from_dict(
            {
                "line_width": 5.0,
                "unknown_key": "ignored",
            }
    ...
    def test_from_dict_empty_dict(self) -> None:
        ...
        spec = SeriesStyleConfig.from_dict({})
        assert spec == SeriesStyleConfig()


class TestSeriesStyleSpecOnFigureSpec:
    """Test SeriesStyleConfig integration with FigureConfig."""

    def test_figure_spec_default_series_styles_empty(self) -> None:
        """FigureConfig default has empty series_styles list."""
    ...
    def test_figure_spec_with_series_styles(self) -> None:
        """FigureConfig accepts list of SeriesStyleConfig."""
        from src.core.models.visualization.figure_config import FigureConfig

        styles = [
            SeriesStyleConfig(line_width=3.0, opacity=0.9),
            SeriesStyleConfig(line_width=1.5, hatching_pattern="/"),
        ]
        spec = FigureConfig(series_styles=styles)
    ...
    def test_figure_spec_round_trip_with_series_styles(self) -> None:
        """FigureConfig with series_styles round-trips through dict."""
        from src.core.models.visualization.figure_config import FigureConfig

        styles = [
            SeriesStyleConfig(opacity=0.6, bar_border_width=1.0),
            SeriesStyleConfig(marker_size=10),
        ]
        spec = FigureConfig(series_styles=styles, title="Test")
    ...
    def test_figure_spec_round_trip_without_series_styles(self) -> None:
        """FigureConfig without series_styles round-trips as empty list."""
        from src.core.models.visualization.figure_config import FigureConfig

        spec = FigureConfig(title="No Styles")
        restored = FigureConfig.from_dict(spec.to_dict())

...
def test_invalid_config_detection():
    ...
    assert len(errors) > 0
    assert any("parseConfig" in error for error in errors)
...
def test_seeds_reducer_enable():
    ...
    assert config["dataManagers"]["seedsReducer"] == True
...
def test_outlier_removal_config():
    ...
    assert config["dataManagers"]["outlierRemover"]["enabled"] == True
...
def test_normalizer_config():
    ...
    assert config["dataManagers"]["normalizer"]["enabled"] == True

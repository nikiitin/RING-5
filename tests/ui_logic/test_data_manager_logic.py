...
@pytest.fixture
def mock_streamlit():
    with patch("src.web.ui.data_managers.seeds_reducer.st") as mock_st_seeds, \
         patch("src.web.ui.data_managers.outlier_remover.st") as mock_st_outlier:
...
def test_seeds_reducer_apply(mock_streamlit, sample_data):
    ...
    with patch("src.web.services.data_processing_service.DataProcessingService.reduce_seeds") as mock_reduce:
        mock_reduce.return_value = pd.DataFrame({"benchmark": ["b1"], "value": [11], "value.sd": [1.4]})
...
def test_outlier_remover_run(mock_streamlit, sample_data):
    ...
    with patch("src.web.services.data_processing_service.DataProcessingService.remove_outliers") as mock_remove:

...
def test_portfolio_serialization():
    ...
    sample_plot = {
        ...
        'name': 'Test Plot',
        'pipeline': [
            {'type': 'columnSelector', 'config': {'columns': ['col1', 'col2']}, 'id': 0}
        ],
        'pipeline_counter': 1,
    ...
    if isinstance(loaded_plot['processed_data'], str):
        loaded_plot['processed_data'] = pd.read_csv(pd.io.common.StringIO(loaded_plot['processed_data']))
    ...
    assert loaded_plot['plot_config']['show_error_bars'] == True

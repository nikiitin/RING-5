...
## 3. Pytest Mastery (*Python Testing with pytest*)
...
*   **Boundaries:** Only mock types you *own*. Wrap external 3rd party libraries (like requests/boto3) in a Facade/Adapter, and mock the Adapter.
...
*   **Exclusions:** Use `# pragma: no cover` for `__main__` blocks and abstract methods. This is a *conscious* decision, not laziness.
...
*   **Refactoring Trap:** Mocks test *Implementation*, not *Behavior*. Prefer real objects with temp DBs for "Integration Lite" tests when possible.
...
*   **Invariants:** Test for:
    ...
    *   *Crash Safety:* Function should not crash for any generated input.

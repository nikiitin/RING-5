...
-   **Concept**: Objects defined by *who* they are, not just *what* they contain.
-   **Gem5 Application**:
...
-   **Gem5 Application**:
    ...
    -   **Rule**: Use `@dataclass(frozen=True)`. Operations (math) return *new* objects.
...
-   **Gem5 Application**:
    -   **The `Experiment` Aggregate**:
        -   *Invariant*: All runs in an experiment must represent comparable workloads (e.g., you can't average `spec_gcc` with `spec_mcf`).
        -   *Rule*: You cannot add a `SimulationRun` to an `Experiment` if it violates the consistency rules (e.g., different Gem5 version).
...
-   **Gem5 Application**:
    -   *Scenario*: Parsing 50 runs and updating the global cache.
...
-   **Goal**: Verify the *flow*. "Given these files exist, when I ask for a comparison, do I get the right result?"

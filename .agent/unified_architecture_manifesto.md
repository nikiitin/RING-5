...
1.  [Preamble: The Pursuit of Scientific Rigor](#preamble)
2.  [Part I: The Core Philosophy - Structural Integrity](#part-i)
    *   [The Domain-Driven Design Imperative](#ddd)
    *   [The Repository Pattern](#repository)
    *   [The Unit of Work](#uow)
    *   [The Message Bus](#messagebus)
3.  [Part II: The Data Engine - High-Performance Analysis](#part-ii)
    *   [The Laws of Vectorization](#vectorization)
    *   [Memory Layout and Strides](#memory)
    *   [The "Tidy Data" Principle](#tidy-data)
    *   [Split-Apply-Combine](#split-apply-combine)
4.  [Part III: The Quality Shield - Verification and Validation](#part-iii)
    *   [The Testing Pyramid](#pyramid)
    *   [Fixtures as Architecture](#fixtures)
    *   [Mocking Strategies](#mocking)
5.  [Part IV: The Ideal Application Blueprint](#part-iv)
...
# <a name="preamble"></a>Preamble: The Pursuit of Scientific Rigor in Software
...
> "The software architecture of a system is the set of structures needed to reason about the system." — *Architecture Patterns with Python*
...
1.  **Structural Integrity:** *Architecture Patterns with Python* (Percival & Gregory). This provides the "Skeleton" (DDD, Clean Architecture).
2.  **Data Proficiency:** *Python for Data Analysis* (McKinney). This provides the "Muscle" (NumPy, Pandas, Vectorization).
3.  **Verification Discipline:** *Test-Driven Development with Python* (Percival). This provides the "Immune System" (Red-Green-Refactor).
4.  **Testing Strategy:** *Python Testing with pytest* (Okken). This provides the "Nervous System" (Fixtures, Parametrization).
...
# <a name="part-i"></a>Part I: The Core Philosophy - Structural Integrity

## <a name="ddd"></a>1.1 The Domain-Driven Design (DDD) Imperative
...
*   *Rule:* Prefer Value Objects over primitives. Never pass raw `int` for cycles; pass `Ticks(1000)`.
...
*   *Example:* `Simulation` is an Aggregate Root. `StatMetric` is internal to it.
*   *Rule:* You cannot delete a `StatMetric` directly. You must ask the `Simulation` to remove it. This ensures the `Simulation` can update its internal caches or re-validate its state.

## <a name="repository"></a>1.2 The Repository Pattern: Abstracting Storage
...
## <a name="uow"></a>1.3 The Unit of Work (UoW): Atomic Operations
...
## <a name="messagebus"></a>1.4 The Message Bus: Decoupling via Events
...
# <a name="part-ii"></a>Part II: The Data Engine - High-Performance Analysis

> "Data analysis is 80% cleaning and preparation." — *Python for Data Analysis*

While Part I defines the *structure*, Part II defines the *muscle*. For Gem5 analysis, we deal with gigabytes of trace data. Standard Python objects (`list`, `dict`) are memory hogs and slow. `for` loops are the enemy. We must think in **Vectors**.

## <a name="vectorization"></a>2.1 The Laws of Vectorization
...
## <a name="memory"></a>2.2 Memory Layout and Strides
...
## <a name="tidy-data"></a>2.3 The "Tidy Data" Principle
...
## <a name="split-apply-combine"></a>2.4 The Split-Apply-Combine Paradigm
...
# <a name="part-iii"></a>Part III: The Quality Shield - Verification and Validation

> "Code without tests is broken by design." — *Test-Driven Development with Python*
...
## <a name="pyramid"></a>3.1 The Testing Pyramid
...
1.  **Unit Tests (70%):**
    ...
    *   **Speed:** < 1ms per test.
    *   **Location:** `tests/unit/`
    *   *Purpose:* Verify logic and edge cases.
...
2.  **Integration Tests (20%):**
    ...
    *   *Purpose:* Verify wiring and schema compatibility.
...
3.  **End-to-End (E2E) Tests (10%):**
    ...
    *   **Location:** `tests/e2e/`
    *   *Purpose:* Verify "Happy Path" user flows.

## <a name="fixtures"></a>3.2 Fixtures as Architecture
...
This ensures Unit tests *cannot* accidentally use the real Database fixture. The namespace protects us.

## <a name="mocking"></a>3.3 Mocking Strategies: London vs. Chicago

### The London School (Mockist)
*   **Philosophy:** Isolate the Unit Under Test (UUT). Mock *everything* it touches.
...
*   *Bad:* `@patch('src.service.Simulation.calculate_ipc')`
*   *Why:* You are mocking a private implementation detail. If you move `calculate_ipc` to a helper function, test breaks.
*   *Rule:* Only mock **Interfaces** (Abstract Base Classes), not concrete implementations.

---

# <a name="part-iv"></a>Part IV: The Ideal Application Blueprint
...
## <a name="directory"></a>4.1 Directory Structure
...
## <a name="golden-path"></a>4.2 The Golden Path (The Life of a Request)
...
2.  **Service Call:** UI calls `messagebus.handle(command)`.
    *   *Crucially: UI does not parse the file. UI does not speak to DB.*
...
## <a name="config"></a>4.3 Configuration Strategy
...
**Signed,**
*The Antigravity Architectural Council*

---

# <a name="appendix-a"></a>Appendix A: The Python Performance Handbook

> "Premature optimization is the root of all evil." — Donald Knuth

However, *late* optimization is impossible if the architecture is wrong.
...
1.  [ ] **Load Less Data:** Use `usecols` in `read_csv`.
    *   *Why:* Loading unused columns wastes RAM/CPU.
2.  [ ] **Downcast Types:** Use `float32` instead of `float64` if precision allows. Use `category` for strings with low cardinality.
    *   *Code:* `df['Status'] = df['Status'].astype('category')`
    *   *Impact:* 10x memory reduction for string columns.
3.  [ ] **Vectorize Strings:** Use `df['str_col'].str.contains()`? **No.**
    *   *Faster:* Convert to category first, or use list comprehensions for complex regex if vectorization is slow.
4.  [ ] **Avoid `apply(axis=1)`:** It is a loop in Python.
    *   *Alternative:* `df['A'] + df['B']` (Vectorized).
    *   *Alternative:* `np.vectorize(func)(df['A'], df['B'])`.
...
# <a name="appendix-b"></a>Appendix B: The Anti-Pattern Bestiary
...
*   **The Fix:** Move logic *into* the Entity. "Tell, Don't Ask". Instead of `service.calculate_speedup(sim)`, use `sim.calculate_speedup()`.
...
# <a name="appendix-c"></a>Appendix C: Security & Reproducibility in Science

## C.1 The Crisis of Reproducibility

If you run the Simulation twice, you must get the *exact same bits*.

### The Seed Strategy
Gem5 (and Python's `random`) are deterministic *if seeded*.
...
# <a name="appendix-d"></a>Appendix D: The Refactoring Guide
...
*   **Action:** Select a block of code. Move it to a new function. Name it after *what* it does, not *how* it does it.
...
3.  **Write Characterization Test:** Write a test that asserts the *current* behavior (even buggy).
...
# <a name="appendix-f"></a>Appendix F: The Glossary

Definitions for the vocabulary of the *Antigravity Architectural Council*.
...
# <a name="appendix-g"></a>Appendix G: The Zen of Python (Architecture Edition)

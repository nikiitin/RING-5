...
def _merge_distribution_ranges(
    target: Dict[str, Any], 
    source: Dict[str, Any]
) -> None:
    """
    ...
    if "minimum" in source:
        ...
        target["minimum"] = (
            min(cur_min, source_min) if cur_min is not None else source_min
        )
    if "maximum" in source:
        cur_max: Optional[float] = target.get("maximum")
        source_max: float = source["maximum"]
        target["maximum"] = (
            max(cur_max, source_max) if cur_max is not None else source_max
        )

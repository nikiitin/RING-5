...
from src.parsers.workers import Gem5ParseWork, ParseWorkPool
...
class SimpleStatsStrategy:
    ...
    def get_work_items(
        ...
        var_map = self._map_variables(variables)
        return [Gem5ParseWork(str(file_path), var_map) for file_path in files]
    ...
    def _map_variables(self, variables: Sequence[StatConfig]) -> Dict[str, Any]:
        ...
        for var in variables:
            ...
            if parsed_ids:
                var_dict["repeat"] = len(parsed_ids)
                stat_obj = TypeMapper.create_stat(var_dict)
            else:
                stat_obj = TypeMapper.create_stat(var_dict)

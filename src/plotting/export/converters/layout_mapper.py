...
class LayoutMapper:
    ...
    def extract_layout(figure: go.Figure) -> Dict[str, Any]:
        ...
        # Get axis label texts to filter them from annotations
        ...
        # Also check if y-axis label is stored as a rotated annotation
        # Look for annotation with textangle=-90 and xref='paper', x near 0
        y_label_from_annotation = None
        if layout.annotations:
            for ann in layout.annotations:
                ...
                if (
                    ...
                    y_label_from_annotation = ann.text
                    y_label_text = ann.text  # Mark for filtering
                    break

        # If no yaxis.title but found annotation, use it as y_label
        if not layout_dict.get("y_label") and y_label_from_annotation:
            layout_dict["y_label"] = y_label_from_annotation
        ...
        if layout.annotations:
            ...
            for ann in layout.annotations:
                ...
                # Check if the annotation text matches the y-axis title
                ...
                # Plotly uses 'x', 'y' for data coords, 'paper' for axes fraction

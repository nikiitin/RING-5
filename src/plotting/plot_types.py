...
class GroupedStackedBarPlot(BasePlot):
    ...
    def render_config_ui(self, data: pd.DataFrame, saved_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with st.expander("Label Renaming"):
            ...
            for y_col in y_columns:
                ...
                new_label = st.text_input(f"Rename '{y_col}'", value=default_label, key=f"legend_rename_{self.plot_id}_{y_col}")
                if new_label != y_col:
                ...
                if len(unique_x) > 50:
                    ...
                    for i, val in enumerate(unique_x):
                        with cols[i % 2]:
                            ...
                            new_val = st.text_input(f"'{val}' →", value=default_val, key=f"x_rename_{self.plot_id}_{val}")
                            if new_val != val:
                ...
                if len(unique_g) > 50:
                    ...
                    for i, val in enumerate(unique_g):
                        with cols[i % 2]:
                            ...
                            new_val = st.text_input(f"'{val}' →", value=default_val, key=f"group_rename_{self.plot_id}_{val}")
                            if new_val != val:
    ...
    def create_figure(self, data: pd.DataFrame, config: Dict[str, Any]) -> go.Figure:
        ...
        fig.update_layout(
            ...
            xaxis_title=config.get('xlabel', x_col) if not group_col else None, # Hide default title if grouped? No, keep it.

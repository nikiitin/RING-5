...
class StatsScanner:
    ...
    DIST_ENTRY_PATTERN = f'(?:{DIST_ENTRY_NUMERIC_PATTERN}|{DIST_ENTRY_OVERFLOW_PATTERN}|{DIST_ENTRY_UNDERFLOW_PATTERN})'
    ...
    SCALAR_REGEX = re.compile(rf'^({VAR_NAME_PATTERN})\s+{SCALAR_VALUE_PATTERN}\s+{COMMENT_PATTERN}?$')
    
    # Distributions
    DIST_REGEX = re.compile(rf'^({VAR_NAME_PATTERN}){DIST_ENTRY_PATTERN}\s+{COMPLEX_VALUE_PATTERN}\s+{COMMENT_PATTERN}?$')
    HISTOGRAM_REGEX = re.compile(rf'^({VAR_NAME_PATTERN}){HISTOGRAM_ENTRY_RANGE_PATTERN}\s+{COMPLEX_VALUE_PATTERN}\s+{COMMENT_PATTERN}?$')
    
    # Summaries (treated as part of vector or distribution usually, but here we want base name)
    SUMMARY_REGEX = re.compile(rf'^({VAR_NAME_PATTERN}){SUMMARIES_ENTRY_PATTERN}\s+{SCALAR_VALUE_PATTERN}\s+{COMMENT_PATTERN}?$')
    
    # Vectors
    # Note: Vector regex in Perl was: ^$varNameRegex$vectorEntryRegex\s+$complexValueRegex\s+$commentRegex?$
    ...
    VECTOR_REGEX = re.compile(rf'^({VAR_NAME_PATTERN}){VECTOR_ENTRY_PATTERN}\s+(?:{COMPLEX_VALUE_PATTERN}|{SCALAR_VALUE_PATTERN})\s*{COMMENT_PATTERN}?$')
        ...
        try:
            with open(file_path, 'r') as f:
                for line in f:
                    ...
                    match = StatsScanner.DIST_REGEX.match(line) or StatsScanner.HISTOGRAM_REGEX.match(line)
                    if match:
                        name = match.group(1)
                        if name not in discovered_vars:

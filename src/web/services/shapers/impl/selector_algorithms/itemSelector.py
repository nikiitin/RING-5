...
class ItemSelector(Selector):
    ...
    def _verifyPreconditions(self, data_frame: pd.DataFrame) -> bool:
        ...
        if not any(
            data_frame[self._column].astype(str).str.contains("|".join(self._strings), na=False)
        ):
            print(
                "Warning: None of the provided strings are present in the DataFrame at the "
                "specified column. No items will be selected."
            )
            verified = False
        ...
        for string in self._strings:
            if not data_frame[self._column].astype(str).str.contains(string, na=False).any():
                print(
                    f"Warning: The string '{string}' is not present in the DataFrame at the "
                    "specified column."
                )
        return verified
...
def test():
    ...
    params = {"column": "system_id", "strings": ["S1", "S3"]}
    print("input: ")
    print(df)

...
Handles CRUD operations for parser variables (scalar, vector, distribution, histogram, configuration).
...
class VariableService:
    ...
    def update_variable(
        ...
        Examples:
            ...
            >>> vars = service.update_variable(vars, 0, {"name": "new", "type": "vector", "_id": "123"})
            >>> vars[0]["name"]
            'new'
        """
    ...
    def aggregate_discovered_entries(
        ...
        for var in snapshot:
            var_name_match = var["name"] == var_name
            try:
                var_name_match = var_name_match or bool(re.fullmatch(var_name, var["name"]))
            except re.error:
                pass

            if var_name_match and "entries" in var:
    ...
    def aggregate_distribution_range(
        ...
        for var in snapshot:
            ...
            if var_name_match and var.get("type") == "distribution":
                m = var.get("minimum")
                M = var.get("maximum")

...
def _normalize_plotly_svg(data: bytes) -> bytes:
    ...
    - Colorbars carry a random 8-hex class token (``cb<8hex>`` /
      ``ycb<8hex>tick``), remapped the same way.
    """
    ...
    # Stable remap of gradient suffixes (order of first appearance).
    suffixes: list[bytes] = []
    for grad in re.finditer(rb"g000000-([0-9a-f]{8})\b", data):
        if grad.group(1) not in suffixes:
            suffixes.append(grad.group(1))
    for index, suffix in enumerate(suffixes):
        data = data.replace(b"g000000-" + suffix, b"g000000-" + b"%08d" % index)

    # Stable remap of colorbar class tokens (6-hex, independent of the main
    # uid): `class="cb<tok> colorbar"` and `class="ycb<tok>tick"` — matched
    # only in those two contexts.
    ...
    for index, token in enumerate(cb_tokens):
        replacement = b"cb" + b"%06d" % index
        data = re.sub(
            rb'(?P<pre>"|y)cb' + re.escape(token),
            rb"\g<pre>" + replacement,
            data,
        )
    return data

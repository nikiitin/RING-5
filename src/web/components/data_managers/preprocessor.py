        ...
        **Preprocessor** creates new columns by combining existing ones with mathematical
        ...
        if loaded is not None:
            ...
            if op_str in operators:
                self._restore_selection("preprocessor", "op", op_str)
            src_cols = loaded["source_columns"]
            valid_src = [c for c in src_cols if c in numeric_cols]
            self._warn_removed_columns([c for c in src_cols if c not in numeric_cols])
            if len(valid_src) >= 1:
                self._restore_selection("preprocessor", "src1", valid_src[0])
            if len(valid_src) >= 2:
                self._restore_selection("preprocessor", "src2", valid_src[1])
            if loaded["dest_columns"]:
                self._restore_selection("preprocessor", "name", loaded["dest_columns"][0])

...
def configure_shaper(shaper_type, data, shaper_id, existing_config, owner_id=None):
    ...
    if shaper_type == "columnSelector":
        ...
        config = {"type": "columnSelector", "columns": selected_columns if selected_columns else []}
        ...
        if (
            normalizer_vars
            and normalize_vars
            and normalizer_column
            and normalizer_value
            and group_by
        ):
            config = {
                "type": "normalize",
        ...
        sort_col_index = (
            categorical_cols.index(sort_col_default) if sort_col_default in categorical_cols else 0
        )
        ...
        if sort_column:
            ...
            if missing:
                ...
                if st.button(
                    "Add all missing values", key=f"{key_prefix}sort_add_missing_{shaper_id}"
                ):
            ...
            if order_list:
                config = {"type": "sort", "order_dict": {sort_column: order_list}}

...
def show_portfolio_page(api: ApplicationAPI) -> None:
    ...
    # The entire page content is wrapped in a single @st.fragment so that
    # interactions on this page (typing a portfolio name, clicking buttons)
    # only re-execute this fragment, not the full app script + sidebar +
    # data preview.
    @st.fragment
    def _portfolio_fragment() -> None:
        ...
        with col2:
            ...
            st.markdown(
                "Restore a previously saved portfolio with all data and plot configurations."
            )

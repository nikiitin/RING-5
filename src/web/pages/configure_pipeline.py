...
class ConfigurePipelinePage:
    ...
    def render(self):
        ...
        st.markdown(
            AppStyles.step_header("Step 3: Configure Processing Pipeline"), unsafe_allow_html=True
        )
    ...
    def _show_pipeline_editor(self):
        ...
        for idx, shaper in enumerate(pipeline):
            with st.expander(
                f"Step {idx + 1}: {self._get_shaper_display_name(shaper)}", expanded=True
            ):
    ...
    def _add_column_selector(self):
        ...
        selected_columns = st.multiselect(
            "Select columns to keep", options=data.columns.tolist(), key="col_selector_cols"
        )

        if st.button("Add Column Selector", key="add_col_selector"):
            if selected_columns:
                shaper = {"type": "columnSelector", "columns": selected_columns}
    ...
    def _add_normalizer(self):
        ...
        with col1:
            normalize_vars = st.multiselect(
                "Variables to normalize", options=numeric_cols, key="norm_vars"
            )

            normalizer_column = st.selectbox(
                "Normalizer column", options=categorical_cols, key="norm_col"
            )
        ...
        with col2:
            if normalizer_column:
                ...
                group_by = st.multiselect("Group by", options=categorical_cols, key="norm_group")
    ...
    def _add_mean_calculator(self):
        ...
        with col1:
            mean_algorithm = st.selectbox(
                "Mean type", options=["arithmean", "geomean", "harmean"], key="mean_algo"
            )

        with col2:
            mean_vars = st.multiselect("Variables", options=numeric_cols, key="mean_vars")

        with col3:
            grouping_column = st.selectbox("Group by", options=categorical_cols, key="mean_group")

        replacing_column = st.selectbox(
            "Replacing column", options=categorical_cols, key="mean_replace"
        )
    ...
    def _add_sort(self):
        ...
        sort_column = st.selectbox("Column to sort", options=categorical_cols, key="sort_col")
        ...
        if sort_column:
            ...
            if st.button("Add Sort", key="add_sort"):
                ...
                if order_list:
                    shaper = {"type": "sort", "order_dict": {sort_column: order_list}}
                    st.session_state.pipeline.append(shaper)
    ...
    def _save_pipeline(self):
        ...
        config_description = st.text_area(
            "Description", value="My custom pipeline", key="save_desc"
        )

        if st.button("Save Now", key="save_now"):
            try:
                self.facade.save_configuration(
                    name=config_name,

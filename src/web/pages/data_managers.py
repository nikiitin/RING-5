...
class DataManagersPage:
    ...
    def render(self):
        ...
        st.markdown(AppStyles.step_header("Data Managers & Transformations"), 
    ...
    def _show_data_visualization(self, data: pd.DataFrame):
        ...
        # Pagination
        st.markdown("---")
        st.markdown(f"### Data Table ({len(display_data)} rows × {len(display_data.columns)} columns)")
        ...
        if rows_per_page == "All":
            ...
            st.info(f"Showing rows {start_idx + 1} to {end_idx} of {total_rows} (Page {page}/{total_pages})")
    ...
    def _show_seeds_reducer(self):
        ...
        st.info("""
        **Seeds Reducer** groups data by categorical columns and calculates statistics across different random seeds.
        ...
        if 'random_seed' not in data.columns:
            st.warning("No `random_seed` column found in the dataset. Seeds Reducer requires this column.")
            st.info("If your data has seeds in a different column, please rename it to `random_seed` first.")
            return
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.markdown(f"**Categorical columns (for grouping):**")
        ...
        with col2:
            st.markdown(f"**Numeric columns (for statistics):**")
        ...
        **Note on Standard Deviation:**
        ...
        - When you later normalize data (in Configure Pipeline), the `.sd` columns will be automatically normalized by the same baseline value
    ...
    def _show_outlier_remover(self):
        ...
        **Outlier Remover** filters out outlier values based on the 3rd quartile (Q3).
            ...
            try:
                ...
                if group_by_cols:
                    filtered_df = self.facade.apply_outlier_remover(
                        ...
                        categorical_cols=group_by_cols
                    )
                else:
                ...
                st.success(f"Removed {removed_count} outlier rows ({(removed_count/len(data)*100):.1f}%)")
        ...
        if 'outlier_result' in st.session_state:
            if st.button("✓ Confirm and Apply Outlier Remover", key="confirm_outlier", type="primary"):
    ...
    def _show_preprocessor(self):
        ...
        st.info("""
        **Preprocessor** creates new columns by combining existing ones with mathematical operations.
        ...
        if 'preproc_result' in st.session_state:
            if st.button("✓ Confirm and Add Column to Dataset", key="confirm_preproc", type="primary"):
                StateManager.set_data(st.session_state['preproc_result'])
                del st.session_state['preproc_result']
                st.success(f"✓ Column added to dataset!")
                st.rerun()
        ...
        if filter_column:
            ...
            if pd.api.types.is_numeric_dtype(col_data):
                ...
                if st.button("Apply Numeric Filter"):
                    filtered_data = data[(data[filter_column] >= filter_min) & 
                                        (data[filter_column] <= filter_max)]
    ...
    def _show_transform_options(self, data: pd.DataFrame):
        ...
        transform_type = st.selectbox(
            ...
            ["Add Column", "Rename Column", "Drop Columns", "Change Data Type", "Fill Missing Values"]
        )
        ...
        if st.button("Fill Missing Values"):
            try:
                ...
                if fill_method == "Mean":
                    ...
                    transformed_data[column].fillna(transformed_data[column].mode()[0], inplace=True)

...
class BackendFacade:
    ...
    def load_csv_pool(self) -> List[Dict[str, Any]]:
        ...
        if self.csv_pool_dir.exists():
            for csv_file in sorted(self.csv_pool_dir.glob('*.csv'), 
    ...
    def load_saved_configs(self) -> List[Dict[str, Any]]:
        ...
        if self.config_pool_dir.exists():
            for config_file in sorted(self.config_pool_dir.glob('*.json'), 
                                     key=lambda x: x.stat().st_mtime, 
                                     reverse=True):
                try:
    ...
    def save_configuration(self, name: str, description: str, 
                          shapers_config: List[Dict], csv_path: Optional[str] = None) -> str:
        """
    ...
    def parse_gem5_stats(self, stats_path: str, stats_pattern: str, 
                        compress: bool, variables: List[Dict],
                        output_dir: str,
        ...
        for var in variables:
            ...
            if var["type"] == "vector":
                if "vectorEntries" in var:
                    ...
                    raise ValueError(f"Vector variable '{var['name']}' requires vectorEntries to be specified")
    ...
    def apply_seeds_reducer(self, data: pd.DataFrame, 
                           categorical_cols: List[str], 
                           statistic_cols: List[str]) -> pd.DataFrame:
        """
    ...
    def apply_outlier_remover(self, data: pd.DataFrame,
                             outlier_column: str,
    ...
    def apply_preprocessor(self, data: pd.DataFrame,
                          operation: str,
                          src_col1: str,
                          src_col2: str,
                          dst_col: str) -> pd.DataFrame:
        """
    ...
    def scan_stats_variables(self, stats_path: str, file_pattern: str = "stats.txt", limit: int = 5) -> List[Dict[str, str]]:
        """
                    ...
                    if name not in merged_vars:
                        ...
                        if var['type'] == 'vector' and 'entries' in var:
                            ...
                            if not new_entries.issubset(existing_entries):
                                merged_vars[name]['entries'] = sorted(list(existing_entries.union(new_entries)))
    ...
    def scan_vector_entries(self, stats_path: str, vector_name: str, file_pattern: str = "stats.txt") -> List[str]:
        """
        ...
        try:
            ...
            # To make it truly fast, we should use grep if available, but let's stick to python for portability.

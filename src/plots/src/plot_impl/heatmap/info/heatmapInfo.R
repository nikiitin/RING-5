...
setMethod("parse_args_plot_info",
  ...
  function(object) {
    ...
    #   stop("Heatmap only allows one faceting variable.")
    # }
...
setMethod("check_data_info_correct",
  ...
  function(object) {
    ...
    if (object@n_y > 1) {
      stop("Heatmap only supports one y axis variable.")
    }
...
setMethod("complete_data",
  signature(object = "Heatmap_info"),
  function(object) {
    for (i in 1:length(object@faceting_var)) {
      object@data[[object@faceting_var[i]]] %<>% factor(levels = unique(object@data[[object@faceting_var[i]]]))
    }
    object@data[[object@conf_z]] %<>% factor(levels = unique(object@data[[object@conf_z]]))
    object@data[[object@x]] %<>% factor(levels = unique(object@data[[object@x]]))
    print(object@data)
    ...
    #   " Filling with NA."))
    # }
    print("Skipping complete_data TODO")
    object
  }
)
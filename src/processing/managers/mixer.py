        ...
        if self._mixerElement:
            ...
            for key, values in self._mixerElement.items():
                ...
                for value in values:
                    ...
                    # Variance_new = Variances_sum (assuming independence)
                    sd_col = f"{value}.sd"
                    if sd_col in DataManager._df.columns:
                        variance = DataManager._df[sd_col] ** 2
                        if mixed_variance is None:
                            mixed_variance = variance
                        else:
                            mixed_variance += variance
                ...
                # Add calculated SD if available
                if mixed_variance is not None:
                    DataManager._df[f"{key}.sd"] = np.sqrt(mixed_variance)
                    DataManager._statistic_columns.append(f"{key}.sd")

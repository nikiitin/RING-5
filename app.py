...
from src.web.ui.shaper_config import configure_shaper, apply_shapers, SHAPER_TYPE_MAP  # noqa: E402
...
def load_csv_data(csv_path: Path) -> Optional[pd.DataFrame]:
    """Load CSV data with automatic separator detection."""
    try:
        return pd.read_csv(csv_path)
    except Exception:
        try:
            return pd.read_csv(csv_path, sep=r'\s+')
        except Exception:
            return None

def update_session_with_data(data: pd.DataFrame, csv_path: str):
    """Update session state with loaded data."""
    st.session_state.data = data
    st.session_state.csv_path = str(csv_path)
    st.session_state.use_parser = False

def _create_parser_config(stats_path: str, stats_pattern: str, compress: bool, 
                          variables: list, output_dir: str) -> DataParserParams:
    """Create parser configuration files and return params object."""
    ...
    return DataParserParams(config_json=parse_config_data)
        ...
        with st.expander(f"{csv_info['name']} ({csv_info['size'] / 1024:.1f} KB)", expanded=(idx==0)):
            ...
            with col1:
                if st.button("Load This File", key=f"load_csv_{idx}"):
                    ...
                    if data is not None:
                        update_session_with_data(data, csv_path)
                        st.success(f"Loaded {len(data)} rows!")
                        st.rerun()
                    else:
                        st.error("Error loading file: Could not parse CSV")
...
def run_parser_with_progress(stats_path: str, stats_pattern: str, compress: bool,
                             ...
                             status_text, file_details) -> Optional[str]:
    ...
    parser_params = _create_parser_config(stats_path, stats_pattern, compress, variables, output_dir)
...
def run_parser(stats_path: str, stats_pattern: str, compress: bool, variables: list) -> Optional[str]:
...
def show_configure_page():
    ...
    **Build a custom pipeline** by adding shapers in any order. You can:
    ...
    with col1:
        # Use the constant map but filter for display options (keys that are capitalized)
        display_options = [k for k in SHAPER_TYPE_MAP.keys() if k[0].isupper()]
        shaper_display = st.selectbox(
            "Select shaper to add",
            options=display_options,
            key='shaper_selector'
        )
        shaper_type = SHAPER_TYPE_MAP[shaper_display]
    with col2:

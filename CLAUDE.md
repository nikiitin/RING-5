...
- Rendering pipeline: `FigureConfig` (`src/core/models/visualization/`) is the single styling source
  ...
  the **exact 15-step `STYLING_PIPELINE_ORDER`** (`src/web/rendering/_connector_protocol.py`):
  backgrounds → font_family → color_palette → title → axis_labels → axis_ticks → axis_ranges →
  axis_colors → grids → legends → reference_lines → data_labels → annotations → hatching →
  margins. (Group separators are not a pipeline step — they are engine-agnostic `SeparatorLine`s
  drawn via `draw_layout_shapes`.) Trace conversion: `trace_to_plotly.py` /

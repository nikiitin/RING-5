...
class Session:
    ...
    def parse_submit(
        ...
        *,
        ...
        out_dir = output_dir or tempfile.mkdtemp(prefix="ring5_parse_")
        try:
    ...
    def shape(self, data: pd.DataFrame, pipeline: list[ShaperStepConfig]) -> pd.DataFrame:
        ...
        try:
            return self.api.apply_shapers(data, pipeline)
    ...
    def reduce_seeds(
        ...
        """
        _require_columns(data, categorical_cols + statistic_cols)
        errors = self.api.managers.validate_seeds_reducer_inputs(
            data, categorical_cols, statistic_cols
        )
        if errors:
            raise DataValidationError("; ".join(errors))
        try:
            return self.api.managers.reduce_seeds(data, categorical_cols, statistic_cols)
    ...
    def remove_outliers(
        ...
        """
        _require_columns(data, [outlier_col] + (group_by_cols or []))
        try:
            return self.api.managers.remove_outliers(data, outlier_col, group_by_cols or [])
        except (ValueError, TypeError) as exc:
            raise DataValidationError(str(exc)) from exc

...
| `src/web/rendering/config_builder.py` | Builders: widgets and Plotly to `FigureConfig` |
...
4. **Sentinel resolution.** `resolve_config(spec)` replaces all `-1`
   sentinel values with inherited parent values. After this step, connectors
   never see `-1`.

5. **Engine selection.** `ChartDisplayComponent.render_engine_selector()`
...
8. **Download.** `render_download_section()` provides export buttons for
   the rendered figure.

...
1. Update `ExportResult` type in `src/web/pages/ui/plotting/export/presets/preset_schema.py`
   if you introduce a new format string.
2. Add a download handler that calls your engine's native export API.
3. Presets (`LaTeXPreset`) are engine-agnostic. Your connector should read
   the same `FigureConfig` fields (DPI, dimensions, font family) that the
   existing connectors use.

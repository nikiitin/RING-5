...
generate figures for a paper, switch to Matplotlib and set the figure dimensions
in the **Layout** pill (for example, Double Column for a two-column paper).
...
**Reuse a pipeline by duplicating its plot.** If you find yourself applying the
same sequence of transformations across multiple plots, build it once and
duplicate that plot — the copy keeps the whole pipeline, which you can then point
at different data. This is especially useful for normalization and
mean-calculation steps that are common across benchmarks.

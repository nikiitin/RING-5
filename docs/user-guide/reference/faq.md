...
Use the export presets feature. Above the settings pills for each plot, you will find a preset selector with options for major venues including IEEE, ACM, ISCA, MICRO, ASPLOS, HPCA, TACO, Nature, and Science.

Selecting a preset automatically adjusts figure dimensions, font sizes, line widths, and DPI to match that venue's style guidelines. Switch to the Matplotlib engine and export as PDF or PGF for the best results in LaTeX documents.
...
For both formats, switch to the Matplotlib engine and select an appropriate export preset for your target venue before downloading.

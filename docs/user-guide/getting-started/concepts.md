...
RING-5 includes 13 export presets that apply conference- and journal-specific formatting in one click. Presets set the figure dimensions, DPI, font sizes, legend spacing, and other presentation-layer properties to match the requirements of a target venue.

Available presets include: ISCA, MICRO, ASPLOS, HPCA, IEEE (single and double column), ACM (single and double column), TACO, Nature, Science, Poster, and Slides.

When you apply a preset, it overrides layout and typography settings while preserving your data, colors, traces, and annotations. You can further adjust any setting after applying a preset.

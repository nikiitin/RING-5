...
publication-quality figure suitable for an ISCA single-column submission. The
figure will be 3.5 inches wide, use serif fonts at 10pt, and export as a
vector PDF or native LaTeX PGF file.

The same workflow applies to any of the 13 built-in presets. RING-5 supports
ISCA, MICRO, ASPLOS, HPCA, TACO, IEEE, ACM, Nature, Science, and presentation
formats. The steps are identical -- only the preset selection changes.
...
## Step 2: Apply an Export Preset

With the Matplotlib engine active, locate the **Preset** selector pills above
the settings controls. You should see a row of preset options including ISCA,
MICRO, ASPLOS, HPCA, and others.

Click **ISCA** to apply the ISCA conference preset.

You should see the chart resize to the ISCA single-column dimensions and the
text elements update to match conference typography requirements.

### What the ISCA Preset Sets

The ISCA preset automatically configures the following properties:
...
The preset also loads a LaTeX preamble that includes the Inconsolata monospace
font (`zi4` package), matching the typewriter font used in typical IEEE/ACM
documents.

Your data, colors, annotations, and trace styling are preserved. Only layout,
typography, and spacing values change.

### Other Conference Presets

If you are submitting to a different venue, select the corresponding preset
instead. The dimensions and typography for each venue are listed in the
[Export Presets](../features/export-presets.md) reference. For example:

- **MICRO** -- 3.5 x 2.5 inches, serif, 10pt base
- **ASPLOS** -- 3.5 x 2.5 inches, serif, 10pt base
- **Nature** -- 3.5 x 3.5 inches (square), Arial, 7pt base, 600 DPI
- **Slides** -- 8.0 x 4.5 inches (16:9), sans-serif, 18pt base

---

## Step 3: Fine-Tune Typography

The preset provides excellent defaults, but you may want to adjust specific
text sizes depending on your chart content.

Click the **Typography** settings pill to open the typography panel. You
should see controls for:

- **Plot title font size** -- defaults to 10 pt from the preset.
- **X-axis title font size** -- defaults to 9 pt.
- **Y-axis title font size** -- defaults to 9 pt.
- **X-axis tick font size** -- defaults to 8 pt.
- **Y-axis tick font size** -- defaults to 8 pt.
...
## Step 4: Adjust Layout and Margins

Click the **Layout** settings pill to open the layout panel.

The preset has already set the width and height to the correct physical
dimensions. You should not change these values unless you have a specific
reason to deviate from the venue requirements.
...
The Matplotlib PDF and PGF export uses `bbox_inches="tight"` by default, which
automatically expands the bounding box to include all visible elements. In most
cases, you will not need to adjust margins manually.
...
- **Axis line width and color** -- the preset configures reasonable defaults,
  but you can darken or thicken axis lines for emphasis.
...
You should see a vector PDF file at exactly 3.5 x 2.5 inches (for the ISCA
preset). Open it in a PDF viewer and zoom in -- all text and lines remain
perfectly sharp because the output is fully vectorized.
...
- Apply the same preset to all figures so that font sizes and dimensions match.
...
The built-in presets already respect these minimums. If you manually adjust
sizes, stay at or above 8 pt for all text.
...
- **PNG** -- useful for including figures in presentation slides or draft
  documents. The ISCA preset exports at 300 DPI, which is sufficient for most
  purposes.
...
1. Switch from Plotly to the Matplotlib engine for static, publication-quality
   output.
2. Apply a conference-specific export preset (ISCA) that sets the correct
   dimensions, font sizes, and typography for your target venue.
...
The full list of available presets and their specifications is in the
[Export Presets](../features/export-presets.md) reference.

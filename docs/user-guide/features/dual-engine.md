...
- **Precise physical dimensions** -- Matplotlib works in inches, so a figure
  sized to a column width in the Layout pill exports at exactly that size.
...
3. **Switch to Matplotlib for export.** Once you are satisfied with the plot
   content, switch to the Matplotlib engine. Set the figure dimensions in the
   Layout settings pill (for example, Double Column for a two-column paper).
